...
export default function OrderForm() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (isMounted) toast.error("Ma'lumotlarni yuklashda xatolik!");
      } finally {
  ...
  const getProductName = (product) =>
    product.name || product.title || product.productName || "Nomsiz mahsulot";

  const getProductPrice = (product) =>
    Number(product.price || product.sellPrice || product.salePrice || 0);

  // =========================================================
  // CART BOSHQRUVI (Unikal ID bo'yicha)
  // =========================================================
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      toast.error("❌ Buyurtmani saqlashda xatolik yuz berdi!");
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-extrabold text-[#6f3518]">
                {orderId ? "Buyurtmani tahrirlash" : "Yangi buyurtma"}
              </h1>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                  {filteredProducts.map((product) => (
                    <button
                      ...
                      className="text-left border border-gray-200 rounded-2xl p-3 bg-white hover:border-[#d97706] hover:shadow-md transition active:scale-[0.98] cursor-pointer flex flex-col justify-between"
                    >
                      <div>
                      ...
                      <div>
                        <p className="text-[#d97706] font-black text-sm mt-2">
                          {getProductPrice(product).toLocaleString()} so'm
                        </p>
                      ...
                      >
                        <div className="flex justify-between gap-2">
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 cursor-pointer flex-shrink-0"
                          >
                            ✕
                          </button>
                        </div>
