...
export default function Analytics() {
  ...
  const getFilteredOrders = () => {
    ...
    return orders.filter((order) => {
      const paidDate = order.paidAt?.toDate
        ? order.paidAt.toDate()
        : order.createdAt?.toDate
        ? order.createdAt.toDate()
        : new Date(order.paidAt || order.createdAt || 0);
    ...
    filteredOrders.forEach((order) => {
      (order.items || []).forEach((item) => {
        if (!dishMap[item.name]) {
          dishMap[item.name] = { name: item.name, count: 0, sum: 0 };
        }
        const qty = item.quantity || 1;
        dishMap[item.name].count += qty;
        dishMap[item.name].sum += (item.price || 0) * qty;
      });
    });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] w-full gap-2">
        ...
        <p className="text-gray-400 text-sm font-medium">Analitika yuklanmoqda...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-8 max-w-6xl mx-auto w-full transition-all pb-28">
      ...
      {/* Sarlavha va Vaqt Filtrini Tanlash */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Kassa Hisobotlari</h1>
