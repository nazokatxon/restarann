...
export default function StaffList() {
  ...
  const openEditModal = (person) => {
    const currentUsername = person.username
      ? person.username
      : person.email
  ...
  const handleSubmit = async (e) => {
    ...
    const fullEmail = `${cleanUsername}@kafe.uz`;
    ...
    try {
      ...
      alert("Xatolik yuz berdi! Ehtimol, bunday login allaqachon mavjud.");
    }
  };
  ...
  const handleDelete = async (staffId) => {
    if (!window.confirm("Bu xodimni o'chirishga ishonchingiz komilmi?")) {
      return;
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((orderDoc) => {
          ...
          if (order.waiterId !== salaryModalPerson.id) return;
      ...
      (error) => {
        console.error("Jami sotuvni hisoblashda xatolik:", error);
        setSalaryForm((prev) => ({
          ...prev,
          totalSales: "0",
        }));
  ...
  const shiftPay =
    Number(salaryForm.shifts || 0) * Number(salaryForm.dailyRate || 0);

  const commission =
    Number(salaryForm.totalSales || 0) *
    (Number(salaryForm.commissionPercent || 0) / 100);

  const deductions =
    Number(salaryForm.advance || 0) + Number(salaryForm.fine || 0);

  const cashToReceive = Math.max(0, shiftPay + commission - deductions);
  ...
  const saveSalaryPayment = async () => {
    ...
    try {
      const history = salaryModalPerson.salaryHistory || [];

      const payment = {
        amount: cashToReceive,
        date: new Date().toISOString(),
        shifts: Number(salaryForm.shifts || 0),
        dailyRate: Number(salaryForm.dailyRate || 0),
        totalSales: Number(salaryForm.totalSales || 0),
        commissionPercent: Number(salaryForm.commissionPercent || 0),
        commission,
        advance: Number(salaryForm.advance || 0),
        fine: Number(salaryForm.fine || 0),
      };

      await updateDoc(doc(db, "users", salaryModalPerson.id), {
        salaryHistory: [...history, payment],
      });
      ...
      console.error("Oylik to'lovini saqlashda xatolik:", error);
      alert("Oylik to'lovini saqlashda xatolik yuz berdi!");
    }
  };
  ...
  const totalSalaries = staff.reduce(
    (sum, person) => sum + (Number(person.salary) || 0),
    0
  );
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64 gap-2">
        <span className="text-sky-500 text-xl animate-spin">⏳</span>
      ...
      <main className="p-4 sm:p-6 max-w-4xl w-full mx-auto flex-1">
        ...
        <div className="flex items-center justify-between mb-6 border-b-2 border-sky-100 pb-4">
          ...
          <button
            ...
            className="group bg-gradient-to-r from-sky-400 to-sky-500 hover:from-sky-500 hover:to-sky-600 text-white px-4 py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-sky-200 hover:shadow-sky-300 active:scale-95 transition-all flex items-center gap-2 cursor-pointer"
          >
            ...
            <div className="space-y-2.5">
              {staff.map((person) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      ...
                      <span className="font-semibold text-gray-700">
                        {Number(person.salary || 0).toLocaleString()} so'm / kun
                      </span>
                    </p>
                  ...
                  <button
                    onClick={() => openSalaryModal(person)}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-5 max-h-[95vh] overflow-y-auto">
            <div className="flex items-start justify-between border-b border-gray-100 pb-3 mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Lavozimi:{" "}
                  {roleLabels[salaryModalPerson.role] ||
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSalaryForm("shifts", e.target.value)
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSalaryForm("dailyRate", e.target.value)
                  }
                />
              </div>
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateSalaryForm("totalSales", e.target.value)
                      }
                    />
                  </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSalaryForm("advance", e.target.value)
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSalaryForm("fine", e.target.value)
                  }
                />
              </div>
            </div>
            ...
            <div className="mt-5 p-4 bg-amber-50/60 border border-amber-200/60 rounded-xl space-y-2 text-xs">
              ...
              <div className="flex justify-between text-red-600">
                <span>Ushlanmalar (Avans/Shtraf):</span>
                <span className="font-semibold">
              ...
              <div className="border-t border-amber-200 pt-2 flex justify-between text-sm font-black text-green-700">
                <span>Qo'lga tegadigan summa:</span>
                <span className="font-semibold">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <input
                  type="text"
                  name="fullName"
                  placeholder="Masalan: Mohira Axmedova"
                  className={inputClass}
                  value={form.fullName}
              ...
              <div className="flex justify-end gap-2 mt-5 pt-3 border-t border-gray-100">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-sky-500 text-white rounded-xl text-xs font-bold hover:bg-sky-600 transition cursor-pointer"
                >
